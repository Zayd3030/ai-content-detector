...
export default function VideoDetection() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setError("Video detection failed. Check backend is running.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-2xl font-semibold text-slate-900">Video Detection</div>
            <div className="mt-1 text-sm text-slate-600">
              Video analysis using frame sampling and aggregation of image-level predictions.
            </div>
          </div>

          <div className="text-xs text-slate-500">
            Backend: <span className="font-mono">/detect/video</span>
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-slate-900">Upload video</div>
            <div className="text-xs text-slate-500">
              {file ? file.name : "No file selected"}
            </div>
          </div>

          <div className="mt-3 rounded-xl border border-slate-900/10 bg-white p-4">
